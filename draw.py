...
def generate_sphere( points, cx, cy, cz, r, step ):
    ...
    while (o < (2*math.pi)):
        x = 0
        while(x < (math.pi)):
            add_point(lines,(r*math.cos(x)+cx),(r*math.sin(x)*math.cos(o)+cy),(r*math.sin(x)*math.sin(o)+cz))
            x+=.2
        o+=.2
    return lines
    pass
...
def add_sphere( points, cx, cy, cz, r, step ):
    lines = generate_sphere(points,cx,cy,cz,r,step)
    for y in lines:
        add_edge(points,y[0],y[1],y[2],y[0]+1,y[1]+1,y[2]+1)
...
def generate_torus( points, cx, cy, cz, r0, r1, step ):
    ...
    while (o < (2*math.pi)):
        x = 0
        while(x < (2*math.pi)):
            add_point(lines,(math.cos(o)*(r0*math.cos(x)+r1)+cx),(r0*math.sin(x)+cy),(-1*math.sin(o)*(r0*math.cos(x)+r1)+cz))
            x+=.4
        o +=.4
    return lines
...
def add_torus( points, cx, cy, cz, r0, r1, step ):
    lines = generate_torus(points,cx,cy,cz,r0,r1,step)
    o = 0
    for y in lines:
        add_edge(points,y[0],y[1],y[2],y[0]+1,y[1]+1,y[2]+1)
    pass
